...
:page-origin: https://wiki.freecad.org/CAM_Post/pl
:page-origin-revision: e402a6b897 2026-07-08
:page-command: CAM_Post
:page-icon: CAM_Post.svg
:page-menu: CAM → Przetwarzanie końcowe
:page-workbench: CAM
:page-shortcut: P P
:page-imported: true
...
--
[.fcTitle]
image:CAM_Post.svg[CAM: Przetwarzanie końcowe,32] *CAM: Przetwarzanie końcowe*

Menu location:: menu:CAM[Przetwarzanie końcowe]
...
Polecenie image:CAM_Post.svg[width=24] xref:output/post-process.adoc[Przetwarzanie końcowe] eksportuje wybrane image:CAM_Job.svg[width=16] xref:job/job.adoc[Zadanie CAM] do pliku G-code.
...
Jeśli chcesz napisać swój własny postprocessor, zobacz stronę xref:output/postprocessor-customization.adoc[CAM: Dostosowywanie przetwarzania końcowego].
...
Postprocesory używają xref:reference/scripting.adoc#wewnętrzny-format-g-code-programu-freecad[wewnętrznego dialektu G-code programu FreeCAD] w połączeniu z definicjami konfiguracji Postprocessora, aby generować G-code zgodny z danym dialektem dla docelowych maszyn. Dzięki temu środowisko pracy CAM we FreeCAD może generować poprawny G-code dla różnych sterowników maszyn CNC, wywołując różne postprocessory.
...
. Wybierz image:CAM_Job.svg[width=16] xref:job/job.adoc[Zadanie CAM] w https://wiki.freecad.org/Tree_view[widoku drzewa].
. Istnieje kilka sposobów na wywołanie tego polecenia:
* Wciśnij przycisk image:CAM_Post.svg[width=16] xref:output/post-process.adoc[Przetwarzanie końcowe].
* Wybierz opcję menu:CAM[Przetwarzanie końcowe] z menu.
* Użyj skrótu klawiaturowego: kbd:[P] a następnie kbd:[P].
. Potwierdź nazwę i katalog dla menu:Pliku wyjściowego[].

== Opcje

Właściwości pliku wyjściowego i postprocessora mogą zostać ustawione w xref:job/job.adoc[Zadaniu] w dowolnej chwili przed wywołaniem postprocessora.
...
* ++**++OUTPUT++_++COMMENTS++**++ (Prawda = Zezwól, Fałsz = Pomijaj): Służy do wstawiania komentarzy tekstowych w pliku wyjściowym G-code.
* ++**++OUTPUT++_++HEADER++**++ (Prawda = Zezwól, Fałsz = Pomijaj): Służy do wstawiania nagłówków tekstowych w pliku wyjściowym G-code.
* ++**++OUTPUT++_++LINE++_++NUMBERS++**++ (Prawda = Zezwól, Fałsz = Pomijaj): Służy do wstawiania numerów linii w pliku wyjściowym G-code.
* ++**++SHOW++_++EDITOR++**++ (Prawda = Zezwól, Fałsz = Pomijaj): Służy do wyświetlania wyjściowego kodu G w oknie wyskakującym po uruchomieniu postprocesora.
...
* ++**++MACHINE++_++NAME++**++ (Nazwa docelowej frezarki CNC): Służy do wstawiania etykiety z nazwą maszyny w pliku wyjściowym.
...
* *TOOL++_++CHANGE*: Kod wstawiany przy każdej zmianie narzędzia w zadaniu.

Opcja menu:Edycja[Preferencje > CAM > Preferencje zadania > Zakładka Postprocessora > Postprocessor] służy do ustawienia domyślnego postprocessora wybranego podczas tworzenia zadania. Umożliwia to skonfigurowanie środowiska pracy CAM tak, aby wyświetlał tylko żądane postprocessory i ustawić domyślny.

Ustawienia postprocessingu mogą pochodzić z wybranego postprocessora Zadania lub z wybranej konfiguracji maszyny. Jeśli używana jest konfiguracja maszyny, może ona definiować postprocessor i powiązane opcje danych wyjściowych.
...
* *Nie* używaj opcji menu:Plik[Eksport] w menu do eksportu kodu G-code, powstanie w ten sposób uszkodzony G-code!
