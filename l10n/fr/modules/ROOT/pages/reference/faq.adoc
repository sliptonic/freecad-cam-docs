= CAM FAQ
:page-origin: https://wiki.freecad.org/CAM_FAQ/fr
:page-origin-revision: 6761d173d0 2026-03-04
:page-imported: true

== Combien d'axes l'atelier CAM peut-il traiter?

Pour le moment, CAM Workbench peut gérer jusqu'à 3 axes de fraisage. Actuellement, les capacités du 4ème axe ne sont pas officiellement incluses mais il existe des xref:reference/fourth-axis.adoc[expérimentations].
...
Des informations plus détaillées sont disponible ici xref:index.adoc#hauteurs-et-profondeurs[Hauteurs et profondeurs].

== Quelle est l'utilisation habituelle de SetupSheet?

La xref:job/setup-sheet.adoc[feuille de configuration] est une feuille de calcul spécifique, contenue dans une tâche, modifié dans la vue Propriétés, seulement accessible depuis l'atelier CAM. Elle fournit une méthode pour configurer les éléments d'une tâche en utilisant des valeurs et des expressions de la xref:job/setup-sheet.adoc[feuille de configuration] aux utilisateurs experts.
...
Les opérations telles que xref:operations/pocket-3d.adoc[Évidement 3D], xref:operations/pocket.adoc[Poche] et xref:operations/mill-face.adoc[Surfaçage] mais pas les opérations de contour, ont une option de configuration pour maintenir l'outil vers le bas, dans l'onglet Données de l'éditeur de propriétés.

== Comment puis-je laisser des attaches pour maintenir mes travaux de fraisage?

L'atelier CAM fournit des xref:dressups/tag.adoc[Attaches] à cette fin.

== Qu'est-ce qu'un post-processeur?

Le xref:output/post-process.adoc[post-processeur] est utilisé pour adapter le code de sortie aux contrôleurs CNC cibles pour diverses machines dans leur langage en G-Code.
...
Oui, les xref:reference/preferences.adoc[préférences de CAM] ont une section pour les post-processeurs où vous pouvez sélectionner les post-processeurs à afficher et sélectionner un post par défaut.
...
L'atelier CAM permet de contrôler l'affichage des lignes des trajectoires en faisant basculer la visibilité de la tâche en la sélectionnant dans la https://wiki.freecad.org/Combo_view[vue combinée]. La visibilité des opérations individuelles ou des groupes d'opérations peut ensuite être modifiée à partir de la vue combinée.
...
xref:reference/scripting.adoc[CAM Ecrire un script].
...
. réglez les valeurs de _Tolérance des géométries par défaut_ et _Précision des courbes par défaut_ à de petites valeurs mais pas à 0, par exemple à 0.01mm.
