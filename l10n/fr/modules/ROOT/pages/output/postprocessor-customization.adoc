...
:page-origin: https://wiki.freecad.org/CAM_Postprocessor_Customization/fr
:page-origin-revision: bf70db3a2f 2026-02-04
:page-imported: true
...
Vous pouvez ensuite consulter le fichier et le comparer à la sortie des postprocesseurs existants tels que `linux_cnc_post.py` ou `grbl_post.py` et essayez de les adapter vous-même ou de les télécharger sur le https://forum.freecad.org/viewforum.php?f=15[forum Path/CAM] pour obtenir de l'aide.
...
A titre de comparaison, vous pouvez regarder les post-processeurs qui sont fournis avec votre installation de FreeCAD. Ils sont situés dans le répertoire ++<++chemin++_++vers++_++votre++_++distro++_++FreeCAD++>++/Mod/CAM/Path/Post/scripts. Les postprocesseurs http://linuxcnc.org/[linuxcnc] et https://github.com/grbl/grbl[grbl] sont très utilisés. L'étude de leur code peut donner des indications utiles.
...
En regardant le fichier linux++_++cnc++_++post.py, vous verrez la fonction d'exportation (à partir de la 0.19.20514, elle se trouve à la ligne 156)
...
{empty}1. *Accéder au chemin d'accès à la macro* :

Allez dans le menu menu:Édition[Préférences > Python > Macro]. Notez le chemin d'accès indiqué dans le champ _Macro Path_ (par exemple, `/home/user/snap/freecad/common`).

{empty}2. *Copiez le fichier du post-processeur* :

Placez votre fichier (par exemple, `new_post.py`) *directement* dans le répertoire _Macro Path_ indiqué à l'étape précédente (par exemple, `/home/user/snap/freecad/common/new_post.py`).
...
* image:CAM_Post.svg[width=24] xref:output/post-process.adoc[CAM Post-traitement]
