= CAM Helix
:page-origin: https://wiki.freecad.org/CAM_Helix
:page-origin-revision: dcb2863a01 2026-07-08
:page-command: CAM_Helix
:page-icon: CAM_Helix.svg
:page-menu: CAM → Helix
:page-workbench: CAM
:page-imported: true

NOTE: This page is incomplete; it was marked as unfinished on the wiki.

[.fcinfobox]
--
[.fcTitle]
image:CAM_Helix.svg[CAM Helix,32] *CAM Helix*

Menu location:: menu:CAM[Helix]
Workbenches:: xref:index.adoc[CAM]
Default shortcut:: _None_
Introduced in version:: -
See also:: _None_
--

== Description

The image:CAM_Helix.svg[width=24] xref:operations/helix.adoc[CAM Helix] command appends a helical clearing operation to the Job. Clockwise Helix outputs (G2) G-Code commands. Counterclockwise outputs (G3) G-Code commands. Step Over percentage specifies the concentric step-over as a percentage of the Tool diameter. One or more helical paths will be created at progressively different diameters, to clear the hole.

== Usage

* Select the image:Workbench_CAM.svg[width=16] xref:index.adoc[CAM Workbench].
* Select the image:CAM_Helix.svg[width=24] icon or kbd:[CAM] → image:CAM_Helix.svg[width=24] Helix from the top menu. This opens the image:CAM_Helix.svg[width=24] Helix configuration panel.
* You will be prompted with a *Choose a Tool Controller* pop-up window to select a Tool Controller. In older versions, in the image:CAM_Helix.svg[width=24] Operation tab, choose a Tool controller and confirm by pressing btn:[Apply].
* Open the Base Geometry tab. All available holes compatible with the Helix tool in the Job Model will be selectable for processing. In the https://wiki.freecad.org/3D_View[3D View] select the holes by their edge or wall face and add them with the btn:[Add] button. Check that they appear in the list. Confirm that the list matches the holes that are intended for processing.
* To remove holes select them in the list and press the btn:[Remove] button.
* Ensure Start Depth, Final Depth and the helix pitch and ramp limits are correct, and adjust if not.
* Ensure Safe and Clearance Heights are correct.
* In the Operation tab, specify the helix Starting surface (outside/inside), cutting Direction (climb/conventional), and Step Over percentage.
* Click btn:[Apply] to create or update the path, btn:[OK] to apply and close the panel, or btn:[Cancel] to abort and close the panel.

== Options

*Extra Offset* adds a margin of material to be left unmachined. This is typically to allow a light finishing pass as a separate operation.

[.since]#introduced in 26.3#: *Max Pitch* replaces _Pitch_ of Helix and limits the maximum helix pitch when non-zero.

Helix *Max Ramp Angle* limits the ramp-entry angle when non-zero. The pitch is calculated for each helix radius.

*Start Radius* is not available in the _Helix_ task panel but can be edited directly in the https://wiki.freecad.org/Property_View[Property View].

This is the radius of material remaining at the center. By default it is zero, giving a path where the tool just touches the axis of the hole. This produces a central path cutting a hole twice the size of the tool diameter.

If this parameter is set to a negative value, it can allow a smaller helix radius. For example, a negative radius equal to the tool radius will produce a zero radius helix: a.k.a. plunge cut with the endmill, should the tool be suitable. This will work as a first cut in a multi-pass operation but attempting to make a helix path with a tool the same size as the hole in the model is rejected.

== Comments

* The helix pitch limit is not respected exactly. It is always rounded to give a complete number of turns from Start Depth to Final Depth.
* Similarly Step Over is not respected exactly. It is always rounded to give a number of equal steps.

The feedrate parameter is constant across all cuts and is based on the position of the tool's axis, thus actual feedrate of the cutting edge of the tool can vary considerably between the inner most cut and the outside surface. If the job dimensions produce a path diameter smaller than the tool diameter, this can lead to much faster cutting speeds than the feedrate given for the tool in the tool controller. This may need to considered when selecting "feed and speeds" in the xref:tools/tool-controller.adoc[tool controller] for the job.

Also the current implementation only uses the horizontal tool speed for helix paths, even in the case where a tight spiral maybe nearly a vertical cut.

== Properties

=== Data

Empty

=== View

Empty

== Scripting

See also: https://wiki.freecad.org/FreeCAD_Scripting_Basics[FreeCAD Scripting Basics].

Example:

[source,python]
----
#Place code example here.
----
