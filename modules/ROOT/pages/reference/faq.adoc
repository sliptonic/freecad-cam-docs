...
At the moment, CAM Workbench can handle up to 3 axis milling. Currently, 4th-axis capabilities are not officially included but xref:reference/fourth-axis.adoc[experimental implementations] exist.
...
Operations such as xref:operations/pocket-3d.adoc[Pocket 3D], xref:operations/pocket.adoc[Pocket Shape], and xref:operations/mill-face.adoc[MillFace], but not Contour Operations have a configuration option to keep the tool down, in the Data tab of the Property View.
...
The xref:output/post-process.adoc[Postprocessor] is used to tailor output code to target CNC controllers for various machines, in their G-Code dialect.
...
Yes, the xref:reference/preferences.adoc[CAM preferences] has a section for post processors where you can select which post processors to display and select a default post.
...
CAM Workbench allows control of the display of path lines by toggling the visibility of the Job by selecting it in the https://wiki.freecad.org/Combo_view[Combo View]. The visibility of individual or groups of Operations are then toggled from the Combo View.
...
. set the values for _Default Geometry Tolerance_ and _Default Curve Accuracy_ to small values but not to 0, e.g. to 0.01mm.
. confirm the change
. Restart FreeCAD.
